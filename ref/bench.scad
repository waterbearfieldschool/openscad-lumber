...
// --- side stretcher ---
// low rail across the two legs of one end (17" tip to tip)
module stretcher() {
    color(wood_dark) cube([TWOBY_T, FRAME_WID - 2*TWOBY_T, TWOBY_W]);
}
...
module base_assembly() {
    ...
    // side stretchers bolted across the end-facing faces of each leg pair
    stretcher_x = [leg_x[0] - TWOBY_T, leg_x[1] + TWOBY_W];
    for (x = stretcher_x)
        translate([x, TWOBY_T, STRETCHER_Z])
            stretcher();
}
